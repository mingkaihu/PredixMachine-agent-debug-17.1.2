...
echo "predix.version=17.1.2" >> "$DIRNAME/predix.home.prs"
...
VM_ARGS="-Xms75m -Dmbs.log.custom=com.prosyst.util.log.buffer.BufferedLogger -Dmbs.log.useEventThread=false -Dmbs.log.file.entriesThreshold=0 -Dorg.osgi.framework.bootdelegation=org.bouncycastle.*:com.fazecast.* -Djava.security.egd=file:///dev/urandom -Dpredix.machine.watchdog=$PREDIX_MACHINE_WATCHDOG -Dsun.net.inetaddr.ttl=60"
...
if [ -e "$PREDIX_MACHINE_HOME/machine/lib/jserialcomm/jSerialComm-1.3.11.jar" ]; then
    EXTRA_CP="$EXTRA_CP:../../../lib/jserialcomm/jSerialComm-1.3.11.jar"
fi
...
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/com.ge.dspmicro.machineadapter-opcua-17.1.2.jar" ]; then
    OPCUA_KEYSTORE_PATH=security/opcua_keystore.jks
fi
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/com.ge.dspmicro.opcua-server-17.1.2.jar" ]; then
...
_ARGS="clean"
if [ -n "$MACHINE_DEBUG_ENABLED" ] && [ ${MACHINE_DEBUG_ENABLED} = true ] ; then
    _ARGS="${_ARGS} debug"
fi
. ./server.sh
